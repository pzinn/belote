...
var dirs=["S","W","N","E"]; var cols=["red","green","blue","yellow"]; var prefixes=["cards/","cards-rotated/","cards/","cards-rotated/"];
...
function drawHandCards() {
	...
	if (i!=pos) // don't draw one's cards
		...
		updatePic(document.getElementById(dirs[k]+j),
			  j<gameInfo.numcards[i] ? "images/"+prefixes[k]+cols[i]+"_back.png" : "images/"+prefixes[k]+"cardholder.png");
    }
}
   ...
   for (var i=0; i<4; i++)
	...
	if (i!=pos) // don't draw one's cards
		...
		for (var j=oldgameInfo.numcards[i]; j<gameInfo.numcards[i]; j++)
		    updatePic(document.getElementById(dirs[k]+j),"images/"+prefixes[k]+cols[i]+"_back.png");
	    else
	      for (var j=gameInfo.numcards[i]; j<oldgameInfo.numcards[i]; j++)
		    updatePic(document.getElementById(dirs[k]+j),"images/"+prefixes[k]+"cardholder.png");
    }
}
...
function drawPlayedCards() {
	...
	updatePic(document.getElementById(dirs[k]+"P"), // the played area
		  gameInfo.playedCards[i]>=0 ? "images/"+prefixes[k]+"card"+gameInfo.playedCards[i]+".png"
		  : "images/"+prefixes[k]+"cardholder.png");
    }
}
...
socket.on("hand", function(h) {
	...
	if (i<hand.length)
	{
	    updatePic(cardel,"images/"+prefixes[0]+"card"+hand[i]+".png");
	    cardel.alt=hand[i];
...
function play(c) {
	...
	cardel.src="images/"+prefixes[0]+"card"+c+".png";
